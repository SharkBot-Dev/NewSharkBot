...
export async function GET(
    ...
    try {
        ...
        const settingRes = await fetch(`${BACKEND_URL}/guilds/economy/${guildId}`, {
            cache: 'no-store'
        });
        
        const itemsRes = await fetch(`${BACKEND_URL}/guilds/economy/${guildId}/items`, {
            cache: 'no-store'
        });
        
        if (!settingRes.ok || !itemsRes.ok) {
