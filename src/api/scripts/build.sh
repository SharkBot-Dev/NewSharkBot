...
if [ "$1" = "--dev" ]; then
  ...
  go build -ldflags "\
  -X github.com/SharkBot-Dev/NewSharkBot/api/internal.Version=$VERSION \
  -X github.com/SharkBot-Dev/NewSharkBot/api/internal.GitCommit=$COMMIT \
  -X github.com/SharkBot-Dev/NewSharkBot/api/internal.GitBranch=$BRANCH" \
  cmd/main.go
fi
